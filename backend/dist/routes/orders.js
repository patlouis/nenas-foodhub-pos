...
router.get("/", requireAuth, async (req, res, next) => {
    try {
        ...
        if (q) {
            ...
            const or = [{ cashierName: re }, { "items.name": re }];
...
router.post("/", requireAuth, validateBody(createOrderSchema), async (req, res) => {
    ...
    const orderType = req.body.orderType ?? "sale";
    const staffMealRecipient = req.body.staffMealRecipient || undefined;
    ...
    const decremented = [];
    for (const it of items) {
        const result = await Product.updateOne({ _id: it.productId, stock: { $gte: it.quantity } }, { $inc: { stock: -it.quantity } });
        if (result.modifiedCount === 0) {
            await Promise.all(decremented.map((d) => Product.updateOne({ _id: d.productId }, { $inc: { stock: d.quantity } })));
            return res
                .status(409)
    ...
    try {
        ...
        if (!order)
            throw new Error("Failed to assign order number after 5 attempts");
...
router.patch("/:id/void", requireAuth, requireAdmin, async (req, res) => {
