...
const router = Router();
const listQuerySchema = paginationQuerySchema(100).extend({
...
router.patch("/:id/void", requireAuth, requireAdmin, async (req, res) => {
    ...
    const updated = await StockAdjustment.findByIdAndUpdate(adj._id, { voided: true, voidedAt: new Date(), voidedBy: req.user.sub, voidedByName: req.user.name }, { returnDocument: "after" });
    res.json(updated);
});
