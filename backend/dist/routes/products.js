...
router.post("/", requireAuth, requireAdmin, validateBody(createProductSchema), async (req, res) => {
    try {
        const data = { ...req.body };
        if (!data.sku)
            data.sku = undefined;
...
router.put("/:id", requireAuth, requireAdmin, validateBody(updateProductSchema), async (req, res) => {
    try {
        const data = { ...req.body };
        if (!data.sku)
            data.sku = undefined;
        const product = await Product.findByIdAndUpdate(req.params.id, data, { returnDocument: "after", runValidators: true });
        if (!product)
...
router.patch("/:id/stock", requireAuth, requireAdmin, validateBody(adjustStockSchema), async (req, res) => {
    try {
        const { delta } = req.body;
        const product = await Product.findOneAndUpdate({ _id: req.params.id, ...(delta < 0 ? { stock: { $gte: -delta } } : {}) }, { $inc: { stock: delta } }, { returnDocument: "after" });
        if (!product) {
...
router.post("/:id/wastage", requireAuth, requireAdmin, validateBody(wastageSchema), async (req, res) => {
    try {
        if (!mongoose.isValidObjectId(req.params.id)) {
            return res.status(400).json({ error: "Invalid product id" });
        }
        const { quantity, reason } = req.body;
        const product = await Product.findOneAndUpdate({ _id: req.params.id, stock: { $gte: quantity } }, { $inc: { stock: -quantity } }, { returnDocument: "after" });
        if (!product) {
